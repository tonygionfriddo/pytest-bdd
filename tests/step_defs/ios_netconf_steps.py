...
def setup_nso(nso_context):
    ...
    assert not error
    assert 'csr1000v-nc-1.0.0' in pkg_list
...
def check_cdb_config(nso_context):
    ...
    return_code, message = nso_context.check_sync(device='csr1000v')
    assert return_code == 0

    return_code, message = nso_context.compare_config(device='csr1000v')
    assert return_code == 0
